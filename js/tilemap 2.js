...
const displayWinnings = () => {
    scoredisplay.innerText = "Congratulations! You Won! Click Start to Play Again!";
}

const displayTryAgain = () => {
    scoredisplay.innerText = "Ahhh, No! Try Again!";
...
let getCustomer = () => {
    ...
    } while (customers[customerindex].hasSelected == true);
    
    return customers[customerindex];
}

const displayCustomer = (index) => {
    const customer = getCustomer();
    const customerDisplay = document.getElementById('customer');
    customerDisplay.innerHTML = `<h2 style="color: white">${customer.name}:</h2>
        <p style="color: white">"${customer.dialogue}"</p>`
}
